...
const Service = {
	...
	addProduct(product) {
		const exists = State.order.items.find(
			(item) => item.product_id == product.product_id && item.purchase_id == product.purchase_id,
		);
	...
	calculate() {
		...
		State.order.items.forEach((item) => {
			if (item.is_gift) {
				...
				item.subtotal_amount = Calculator.amount(item.quantity, item.selling_price);

				item.discount_amount = Calculator.min(item.discount_amount, item.subtotal_amount);

				const taxable = Calculator.discount(item.subtotal_amount, item.discount_amount);

				item.vat_amount = Calculator.vat(taxable, item.vat_rate);

				item.total_amount = Calculator.total(taxable, item.vat_amount);
			}
...
================================================= */
	...
	calculateDebt() {
		State.order.debt_amount = Calculator.debt(State.order.total_amount, State.order.paid_amount);
	},
