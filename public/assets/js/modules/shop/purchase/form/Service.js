...
const Service = {
    ...
    async loadPurchase(id) {
        ...
        Object.assign(State.purchase, purchase[0] || {});
    ...
    setMoneyOverall() {
        ...
        for (const item of State.purchase.items) {
            item.subtotal_amount = Calculator.amount(item.quantity, item.purchase_price);

            item.vat_rate = State.purchase.vat_rate;

            item.vat_amount = Calculator.vat(item.subtotal_amount, item.vat_rate);

            item.total_amount = item.subtotal_amount;

            item.total_amount_with_vat = Calculator.total(item.total_amount, item.vat_amount);
        ...
        State.purchase.debt_amount = Calculator.debt(totalAmount, State.purchase.paid_amount);
    },
